...
La cause ? Une modification effectuée la veille sur une politique Conditional Access. Une petite règle, ajoutée *« pour renforcer la sécurité »*. Sauf qu’elle bloque tout le monde.
...
Le souci, c’est que dans de nombreuses organisations, cette responsabilité repose encore sur **une seule personne** (parfois deux). Pas parce que les équipes sont mauvaises, mais parce que le sujet est perçu comme complexe et qu’on a peur de *« casser quelque chose »*.
...
- **Risque opérationnel** : En cas d’absence, de départ ou de problème, l’entreprise peut se retrouver bloquée.
- **Risque d’erreur humaine** : Une modification mal maîtrisée peut impacter des centaines d’utilisateurs.
- **Risque de sécurité paradoxal** : En voulant trop centraliser pour *« mieux contrôler »*, on crée un point de fragilité exploitable (pression, erreur, ou départ de la personne clé).
...
Toute modification importante sur Conditional Access doit passer par un circuit clair : test → revue → validation → documentation.
...
Utilisez les rôles Entra ID les plus précis possibles, des groupes de sécurité dédiés, et les fonctionnalités de *« report-only »* pour tester les changements sans impact.

### 4. Documenter et versionner
Traitez vos politiques Conditional Access comme du code : commentez-les, expliquez le *« pourquoi »* de chaque règle, et gardez un historique des modifications.

> 💡 **Astuce** : La règle simple : **personne ne devrait être irremplaçable** sur un sujet qui peut bloquer l’accès à toute l’entreprise.
