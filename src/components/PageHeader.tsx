...
export default function PageHeader({
  ...
  return (
    <header
      className={cn(
        "relative border-b border-border-main surface-header overflow-hidden",
          ...
          <>
            {title && (
              <motion.h1
                ...
                className="flex flex-wrap items-center gap-x-3 gap-y-2 text-2xl sm:text-3xl md:text-4xl font-bold text-text-primary tracking-tight"
              >
                {showPrefix && (
            ...
            {title && <div className="heading-accent" aria-hidden />}
            {subtitle && (
              <motion.p
                initial={{ opacity: 0, y: 16 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: 0.08 }}
                className="mt-4 text-text-secondary text-sm sm:text-base font-medium max-w-2xl leading-relaxed"
              >
                {subtitle}
              </motion.p>
            )}
