...
function HeroDashboard({
  ...
  return (
    <div className="relative aspect-square max-w-[340px] ml-auto w-full">
      <div className="absolute inset-0 bg-bg-secondary/40 border border-white/5 backdrop-blur-3xl rounded-[1.5rem] shadow-2xl overflow-hidden flex flex-col">
        {/* Mac Title Bar */}
        <div className="bg-navy/60 border-b border-white/5 px-4 py-2.5 flex items-center justify-between">
          <div className="flex space-x-2">
            <div className="w-2 h-2 rounded-full bg-[#ff5f57]"></div>
            <div className="w-2 h-2 rounded-full bg-[#febc2e]"></div>
            <div className="w-2 h-2 rounded-full bg-[#28c840]"></div>
          </div>
          <div className="text-[8px] font-mono text-text-secondary/50 uppercase tracking-widest font-bold">
        ...
        <div className="flex-grow relative overflow-hidden bg-navy/10">
          <AnimatePresence mode="wait">
            {dashboardMode === "mesh" ? (
              ...
              <motion.div key="terminal-mode" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="absolute inset-0 p-5 font-mono text-[8px]">
                <div className="space-y-1">
                  {visibleLogs.map((log, idx) => (
                    <div key={idx} className={log.includes("FW_BLOCK") ? "text-pink-500" : "text-turquoise/90"}>
                      <span className="opacity-30 mr-1.5">{">"}</span> {log}
                    </div>
                  ))}
                </div>
              </motion.div>
        ...
        {/* Metrics Section */}
        <div className="bg-navy/80 border-t border-white/5 p-4 backdrop-blur-xl">
          <div className="flex justify-between items-center mb-3">
            <div className="flex items-center space-x-2">
              <div className="p-1 bg-turquoise/20 rounded-lg text-turquoise">
                <Activity size={10} />
              </div>
              <span className="text-[7px] font-black text-text-primary uppercase tracking-[0.2em]">{t("hero.monitor_live")}</span>
            </div>
            <div className="flex items-center space-x-1.5 bg-green-500/10 px-2 py-0.5 rounded-md border border-green-500/20">
              <span className="h-1 w-1 rounded-full bg-green-500 animate-pulse"></span>
              <span className="text-[7px] font-black text-green-500 uppercase tracking-widest">{t("hero.monitor_live_badge")}</span>
            </div>
          </div>

          <div className="space-y-3">
            <div>
              <div className="flex justify-between text-[7px] font-bold text-text-secondary/60 mb-1 uppercase tracking-widest">
...
export default function Hero() {
  ...
  return (
    <section className="relative pt-24 pb-8 md:pt-28 md:pb-12 min-h-[60vh] lg:min-h-[75vh] flex items-center overflow-hidden noc-grid">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-16 items-start">
          ...
          <div className="lg:col-span-7 flex flex-col justify-center">
            ...
            <div className="space-y-0.5 mb-8">
              <motion.h1 initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }} className="text-4xl md:text-6xl lg:text-6xl font-black tracking-tighter text-text-primary code-font">
                {t("hero.title_part1")}
              </motion.h1>
            ...
            >
              <a href="#categories" className="px-5 py-2.5 bg-text-primary text-bg-primary font-black rounded-xl flex items-center justify-center transition-all hover:scale-105 active:scale-95 shadow-xl text-[9px] tracking-widest uppercase">
                {t("hero.cta_explore")}
              </a>
              <a href="#" className="px-5 py-2.5 bg-bg-secondary border border-turquoise/50 text-turquoise font-black rounded-xl flex items-center justify-center transition-all hover:bg-turquoise/10 text-[9px] tracking-widest uppercase group shadow-lg shadow-turquoise/5">
                <Crown size={12} className="mr-2 group-hover:rotate-12 transition-transform" />
                {t("hero.cta_premium")}
              </a>
              <a href="#newsletter" className="px-5 py-2.5 bg-bg-secondary border border-border-main text-text-secondary font-bold rounded-xl flex items-center justify-center transition-all hover:bg-turquoise/5 text-[9px] tracking-widest uppercase">
                {t("hero.cta_news")}
              </a>
            </motion.div>
